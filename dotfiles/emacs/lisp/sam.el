...
(defvar sam-printed-regions nil
  "The regions printed by the current command, to create multiple-cursors for afterwards.")
(make-variable-buffer-local 'sam-printed-regions)
...
(defun sam-print (addr)
  (sam-command addr)
  (setq sam-printed-regions
        (cons
         (cons (sam-addr-beg addr) (sam-addr-end addr))
         sam-printed-regions))
...
(defvar sam-current-command nil)

(defun sam-eval-command-for-cursor ()
  (interactive)
  (save-current-buffer
    (eval sam-current-command)))

(defun sam-eval-command (cmd)
  (setq sam-current-command cmd)
  (pp cmd)
  (mc/execute-command-for-all-fake-cursors 'sam-eval-command-for-cursor)
  (sam-eval-command-for-cursor)
  (setq sam-last-command (car cmd))
  (setq sam-current-command nil)
  (save-excursion
    (while sam-affected-buffers
      (let* ((buffer (car sam-affected-buffers))
        ...
        (undo-boundary)
        (when sam-printed-regions
          (mc/remove-fake-cursors)
          (save-mark-and-excursion
            (dolist (region (reverse (cdr sam-printed-regions)))
              (set-mark (car region))
              (goto-char (cdr region))
              (mc/create-fake-cursor-at-point)))
          (setq sam-printed-regions nil)
